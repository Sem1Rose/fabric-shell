...
class PopupWindow(WaylandWindow):
    ...
    def do_calculate_edges(self):
        ...
        if len(parent_anchor) != 3:
            if len(parent_anchor) == 4:
                return move_axe

            if (
                GtkLayerShell.Edge.LEFT in parent_anchor
                or GtkLayerShell.Edge.RIGHT in parent_anchor
            ):
                return "y"
            if (
                GtkLayerShell.Edge.TOP in parent_anchor
                or GtkLayerShell.Edge.BOTTOM in parent_anchor
            ):
                return "x"
    ...
    def do_reposition(self, move_axe: str):
        ...
        # logger.error(f"{height}, {width}")

        if self._pointing_widget:
            coords = self.get_coords_for_widget(self._pointing_widget)
            logger.error(coords)
            logger.error(
                f"{self._pointing_widget.get_allocated_width()}, {self._pointing_widget.get_allocated_height()}"
            )
            coords_centered = (
